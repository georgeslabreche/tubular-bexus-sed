...
%DIF ADD doc/sed/v3.0/main.tex   Wed Jul 11 13:12:28 2018
...
\usepackage{dcolumn}  %DIF > 
  \newcolumntype{d}[1]{D{.}{.}{#1}} %hopefully one of these aligns tables decimal places, natalie %DIF > 
%DIF -------
...
%DIF 135a136-137
\renewcommand*{\appendixsection}{Appendix~\arabic{section}} %DIF > 
 %DIF > 
%DIF -------
...
\begin{itemize}
  ...
  \item \DIFaddbegin \textbf{\DIFadd{SilcoTek}} \DIFadd{provides coatings solutions that are corrosion resistant, inert, H2S resistant, anti-fouling, and low stick.
  }\item \textbf{\DIFadd{Swagelok}} \DIFadd{designs, manufactures, and delivers an expanding range of fluid system products and solutions.
  }\item \DIFaddend \textbf{Teknolab Sorbent} provides products such as  analysis instruments and accessories within reference materials, chromatography and separation technology.
...
\DIFdel{collect data on the humidity.
    }%DIFDELCMD < \item \st{The experiment \textit{shall} measure the temperature inside the AAC Valve Box.}%%%
\item%DIFAUXCMD
\DIFdel{\textsuperscript{\ref{fn:unnecessary-requirement}}
...
\DIFdelend \DIFaddbegin \item[P.13] \DIFaddend The accuracy of temperature measurements \textit{shall} be +3.5/-3$\degree$C (max) for condition of -55$\degree$C to 150$\degree$C.
    \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{The accuracy of the ambient humidity measurements }\textit{\DIFdel{shall}} %DIFAUXCMD
\DIFdel{be ±3\%. \mbox{%DIFAUXCMD
\cite{Humiditysensor}
    }\hspace{0pt}%DIFAUXCMD
}%DIFDELCMD < \item \st{The accuracy of the AAC Valve Box temperature measurements \textit{shall} be +3.5/-2$\degree$C(max).}%%%
\item%DIFAUXCMD
...
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{The sensors }\DIFdelend \DIFaddbegin \item[P.23] \DIFadd{The temperature sensor }\DIFaddend sampling rate \textit{shall} be \DIFdelbegin \DIFdel{2 }\DIFdelend \DIFaddbegin \DIFadd{1 }\DIFaddend Hz.\label{newsamplerate}
    \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
...
\DIFdelend \DIFaddbegin \item[P.25] \DIFaddend The minimum volume of air in the bags for analysis \textit{shall} be 0.18 L at ground level.
    ...
    }\item[P.28] \DIFadd{The pressure sensor sampling rate }\textit{\DIFadd{shall}} \DIFadd{be 1 Hz.}\label{newsamplerate}
    \item[P.29] \DIFadd{The airflow sensor sampling rate }\textit{\DIFadd{shall}} \DIFadd{be 1 Hz.}\label{newsamplerate}
...
\DIFdelbegin %DIFDELCMD < \begin{enumerate}[label=C.\arabic*]
%DIFDELCMD <     \item %%%
\DIFdelend \DIFaddbegin \begin{enumerate}
    \item[C.1] \DIFaddend Constraints specified in the BEXUS User Manual.
\DIFdelbegin %DIFDELCMD < \item \st{The person-hours allocated to project implementation is limited by university related factors such as exams, assignments, and lectures.}%%%
\item%DIFAUXCMD
\DIFdel{\textsuperscript{\ref{fn:unnecessary-requirement}}
    }%DIFDELCMD < \item \st{Budget limited to TBD.}%%%
\item%DIFAUXCMD
\DIFdel{\textsuperscript{\ref{fn:unnecessary-requirement}}
    }%DIFDELCMD < \item \st{The dimensions show a minimum print area of 50 x 50 cm and 65 cm height experiment box.}%%%
\item%DIFAUXCMD
\DIFdel{\textsuperscript{\ref{fn:unnecessary-requirement}}
}\DIFdelend \end{enumerate}
...
\multicolumn{1}{|l|}{Experiment Building and Testing} & \multicolumn{1}{c|}{04/06} & 16/09 & 105 & \DIFdelbeginFL \DIFdelFL{2048 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3072 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{- }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{720* }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{- }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-76.56 }\DIFaddendFL \\ \hline
...
\multicolumn{1}{r}{\textbf{}} & \multicolumn{1}{l}{} & \multicolumn{1}{r|}{\textbf{Total:}} & \textbf{353} & \textbf{6282} & \textit{\DIFdelbeginFL \DIFdelFL{1524*}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3727*}\DIFaddendFL } & \textit{\DIFdelbeginFL \DIFdelFL{-118.51}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-40.67}\DIFaddendFL *} \\ \cline{4-7} 
\end{tabular}
...
\input{3-project-planning/tables/masset-highlight.tex}
...
The primary concern regarding the AAC air sampling subsystem occurs after the cut-off when the gondola will tumble and fall at an average speed of 50 m/s for approximately two minutes \cite{BexusManual}. This descent speed is too large in order to sample air at the desired vertical resolution, capped at 500 m. As such, sampling can only be done after the gondola has stabilized at a descent speed  of 8 m/s \cite{BexusManual}. The tumbling phase will vertically span for approximately 6 km. Considering a Float Phase altitude of 25 km, sampling during the Descent Phase will commence at approximately 19 km in altitude. However, the primary region of interest in terms of sampling is in the stratosphere, particularly between 19 km and 25 km in altitude. Sampling will thus also occur during the Ascent Phase. Out of the six sampling bags present in the payload, two will be used during the Ascent Phase at 18 km and 21 km and four during the Descent Phase at 17.5 km, 16 km, 14 km and 12 km as seen in Table \DIFdelbegin \DIFdel{\ref{tab:sampling-altitudes}}\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:minimum-volume}}\DIFaddend . Details regarding the sampling strategy can be found in Appendix \ref{sec:appH}.

\DIFdelbegin %DIFDELCMD < \begin{table}[H]
%DIFDELCMD < \centering
%DIFDELCMD < \begin{tabular}{|c|c|c|c|}
%DIFDELCMD < \hline
%DIFDELCMD < \multicolumn{1}{|l|}{} & \multicolumn{1}{l|}{\textbf{Sampling Altitudes}} & \multicolumn{1}{l|}{\textbf{Ambient Pressure}} & \multicolumn{1}{l|}{\textbf{Ambient Temperature}} \\ \hline
%DIFDELCMD < \multirow{2}{*}{\textbf{Ascent Phase}} & %%%
\DIFdelFL{18 km }%DIFDELCMD < & %%%
\DIFdelFL{75.0 hPa }%DIFDELCMD < & %%%
\DIFdelFL{216.7 K }%DIFDELCMD < \\ \cline{2-4} 
%DIFDELCMD <  & %%%
\DIFdelFL{21 km }%DIFDELCMD < & %%%
\DIFdelFL{46.8 hPa }%DIFDELCMD < & %%%
\DIFdelFL{217.6 K }%DIFDELCMD < \\ \hline
%DIFDELCMD < \multirow{4}{*}{\textbf{Descent Phase}} & %%%
\DIFdelFL{17.5 km }%DIFDELCMD < & %%%
\DIFdelFL{81.2 hPa }%DIFDELCMD < & %%%
\DIFdelFL{216.7 K }%DIFDELCMD < \\ \cline{2-4} 
%DIFDELCMD <  & %%%
\DIFdelFL{16 km }%DIFDELCMD < & %%%
\DIFdelFL{102.9 hPa }%DIFDELCMD < & %%%
\DIFdelFL{216.7 K }%DIFDELCMD < \\ \cline{2-4} 
%DIFDELCMD <  & %%%
\DIFdelFL{14 km }%DIFDELCMD < & %%%
\DIFdelFL{141.0 hPa }%DIFDELCMD < & %%%
\DIFdelFL{216.7 K }%DIFDELCMD < \\ \cline{2-4} 
%DIFDELCMD <  & %%%
\DIFdelFL{12 km }%DIFDELCMD < & %%%
\DIFdelFL{193.3 hPa }%DIFDELCMD < & %%%
\DIFdelFL{216.7 K }%DIFDELCMD < \\ \hline
...
%DIFAUXCMD
%DIFDELCMD < \label{tab:sampling-altitudes}
%DIFDELCMD < \end{table}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > \input{4-experiment-design/tables/samplingaltitudes.tex}
The maximum pressure that the sampling bags can withstand has to be taken into account in order to avoid bursting. Decreasing pressure during the Ascent Phase poses a risk to sampling bags which already contain samples as the gas inside will expand which may cause the bag to burst. In order to avoid this, the sampling bags will not be completely filled. Filling up to a maximum of 80\% of the sampling bag's capacity (2 psi/0.14 bar) is recommended by the manufacturers for the Multi-Layer Foil sampling bags that are to be used. \DIFaddbegin \DIFadd{Therefore, the maximum expected pressure inside the bags, that will be filled during the Ascent Phase, will be 1.6 psi/0.11 bar. }\DIFaddend The inverse is also true for the Descent Phase where compression will occur. As such, the sampling bags should be fully filled during the Descent Phase in order to ensure that enough samples are collected for analysis. \DIFaddbegin \DIFadd{During the Descent Phase, the maximum pressure inside the bags is expected to be 1.98 psi/0.13 bar. }\DIFaddend Past research has revealed that the selected sampling bags can withstand pressure difference of 310 hPa at 30 km of altitude, which is equivalent to 0.31 bar \cite{LISA}. A series of tests listed in Table \ref{tab:sampling-system-test} will be conducted in order to confirm the maximum allowable pressure for the bags.

\DIFaddbegin \DIFadd{The maximum operating pressure for the tubes, according to the manufacturers, is 2.2 psi/0.15 bar. The valve's leakage rate, given by the manufacturers, is 0.001 l/min.     
}


\DIFaddend Due to the difference in pressure between sea level and sampling altitudes, the volume of the sample taken will be considerably reduced when it reaches sea level. This shrinking has to be taken into account as the minimum volume that has to be present in the sampling bag at ground level in order to obtain results with the Picarro analyzer. A minimum amount is required for the analyzer to detect concentrations of the targeted trace gases. This minimum amount is 0.18 L at sea level and it has to be specially considered for the samples taken at higher altitudes. The samples taken at lower altitudes will be exposed to smaller changes in pressure, therefore their size will not be critically reduced. Table \ref{tab:minimum-volume} shows the minimum volume of air that needs to be sampled at different altitudes \DIFaddbegin \DIFadd{and the corresponding temperature and pressure conditions}\DIFaddend , in order the sample volume left at sea level pressure \DIFaddbegin \DIFadd{and temperature (288 K)  }\DIFaddend is at least 0.18L.  
\DIFdelbegin \DIFdel{.  
}\DIFdelend 
...
\end{tabular}
\caption{\DIFaddbeginFL \DIFaddFL{Sampling Altitudes as well as the Corresponding Ambient Pressures and Temperatures According to the 1976 US Standard Atmosphere and the }\DIFaddendFL Minimum Sampling Volume at Each Altitude to Obtain Enough Air to Perform a Proper Analysis (0.18 L at \DIFdelbeginFL \DIFdelFL{sea level}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Sea Level}\DIFaddendFL ), Appendix \ref{sec:appH}\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
The sampling of the AAC will be triggered by the pressure reading from the sensors outside the experiment box. When the required pressure is reached, Table \DIFdelbegin \DIFdel{\ref{tab:sampling-altitudes} }\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:minimum-volume}}\DIFaddend the valve inside the manifold corresponding to the bag that is to be sampled, will open and the sampling will start. The closing of the valve depends on two conditions and it will be triggered when either one of the conditions is true. These conditions are: maximum sampling time or maximum pressure difference between inside/outside the bags. They are determined from past research \cite{LISA}. A first estimation of the maximum sampling time has already been made, from Test 18 shown in Table \ref{tab:pump-low-pressure-test}, but more tests in the future will determine the maximum pressure condition and confirm the maximum sampling times.
...
\smallskip
\DIFaddend A simple but reliable fixing interface between the two boxes of the experiment has been designed to ensure the fast recovery of the CAC box. The latter \DIFdelbegin \DIFdel{implies }\DIFdelend \DIFaddbegin \DIFadd{requires }\DIFaddend only unscrewing 12 bolts as well as unplugging a D-Sub connector. Once the CAC box is detached, the AAC Box will still remain perfectly fixed in the gondola. Table \ref{table:attaching-components} gathers up all the components required to fix the experiment to the gondola.
...
\DIFdel{To Be Ordered }\DIFdelend \DIFaddbegin \DIFadd{Received }\DIFaddend \\ \hline \DIFdelbegin \DIFdel{E47 }\DIFdelend \DIFaddbegin \DIFadd{E51 }\DIFaddend & \DIFdelbegin \DIFdel{25-pin D-SUB backing }\DIFdelend \DIFaddbegin \DIFadd{8-pin male single row header }\DIFaddend & RND Connect & RND \DIFdelbegin \DIFdel{205-00722 }\DIFdelend \DIFaddbegin \DIFadd{205-00629 }\DIFaddend & \DIFaddbegin \DIFadd{5 }& \DIFaddend 1 & \DIFdelbegin \DIFdel{48 }\DIFdelend \DIFaddbegin \DIFadd{0.28 }\DIFaddend &  \DIFdelbegin \DIFdel{6.2 }\DIFdelend & \DIFdelbegin \DIFdel{For connecting distributed components }\DIFdelend \DIFaddbegin \DIFadd{Received }\\ \hline \DIFadd{E52 }\DIFaddend & \DIFdelbegin \DIFdel{To Be Ordered }\DIFdelend \DIFaddbegin \DIFadd{10-pin male single row header }& \DIFadd{Prostar }& \DIFadd{SD-2X5-T1-7/3MM }& \DIFadd{1 }& \DIFadd{1 }& \DIFadd{0.26 }&  & \DIFadd{Received }\DIFaddend \\ \hline \DIFdelbegin \DIFdel{E48 }\DIFdelend \DIFaddbegin \DIFadd{E53 }\DIFaddend & \DIFdelbegin \DIFdel{Power wire - Red }\DIFdelend \DIFaddbegin \DIFadd{36-pin male double row header }\DIFaddend & \DIFdelbegin \DIFdel{Alpha Wire }\DIFdelend \DIFaddbegin \DIFadd{Würth Elektronik }\DIFaddend & \DIFdelbegin \DIFdel{5856 RD005 }\DIFdelend \DIFaddbegin \DIFadd{61303621121 }\DIFaddend & 1 & \DIFdelbegin \DIFdel{370 }\DIFdelend \DIFaddbegin \DIFadd{2 }\DIFaddend & \DIFdelbegin \DIFdel{46 }\DIFdelend \DIFaddbegin \DIFadd{1.7 }\DIFaddend &  \DIFdelbegin \DIFdel{Roll of 30 m. Afifth will be used approximately }\DIFdelend & \DIFdelbegin \DIFdel{To Be Ordered }\DIFdelend \DIFaddbegin \DIFadd{Received }\DIFaddend \\ \hline \DIFdelbegin \DIFdel{E49 }\DIFdelend \DIFaddbegin \DIFadd{E54 }\DIFaddend & \DIFaddbegin \DIFadd{DC/DC converter 24 V }& \DIFadd{Delta }&  & \DIFadd{2 }& \DIFadd{20 }& \DIFadd{34 }& \DIFadd{12V,1.67A, 20W DCDC }& \DIFadd{Received }\\ \hline \DIFadd{E55 }& \DIFaddend Potentiometer \DIFdelbegin \DIFdel{1 }\DIFdelend \DIFaddbegin \DIFadd{50 }\DIFaddend kOhm & Bourns & \DIFdelbegin \DIFdel{M64Y102KB40 }\DIFdelend \DIFaddbegin \DIFadd{3296Y-1-503LF }\DIFaddend & \DIFdelbegin \DIFdel{2 }\DIFdelend \DIFaddbegin \DIFadd{4 }\DIFaddend & 1 & 1.8 &  & \DIFaddbegin \DIFadd{Received }\\ \hline \DIFadd{E56 }&  & \DIFadd{Gems Sensors }& \DIFadd{Controll }& \DIFadd{3500S0001A05E000 }& \DIFadd{1 }& \DIFadd{53 }& \DIFadd{150 }&  & \DIFaddend To Be Ordered \\ \hline \caption{Electrical Components Table} \label{tab:components-table-electrical} \end{longtable} \raggedbottom\DIFaddbegin \DIFadd{"
}\DIFaddend 
...
\DIFdelend \DIFaddbegin \DIFadd{32" to female }\DIFaddend 1\DIFaddbegin \DIFadd{/4") }\DIFaddend & \DIFdelbegin \DIFdel{200 }\DIFdelend \DIFaddbegin \DIFadd{Swagelok }\DIFaddend & \DIFdelbegin \DIFdel{- }\DIFdelend \DIFaddbegin \DIFadd{SS-400-8-4 }\DIFaddend & \DIFdelbegin \DIFdel{Need to be custom ordered }\DIFdelend \DIFaddbegin \DIFadd{2 }\DIFaddend & \DIFdelbegin \DIFdel{To Be Ordered }\DIFdelend \DIFaddbegin \DIFadd{62 }& \DIFadd{23 }&  & \DIFadd{Received }\DIFaddend \\ \hline \DIFdelbegin \DIFdel{M50 }\DIFdelend \DIFaddbegin \DIFadd{M55 }\DIFaddend & Magnesium filter tube with interface & FMI & - & 1 & 65 & 150 &  & Ordered \\ \hline \DIFdelbegin \DIFdel{M51 }%DIFDELCMD < & %%%
...
\DIFadd{The two-box design will allow ease of access and manipulation of both the CAC and AAC subsystems. In addition, the AAC sampling system is designed to be re-usable for future handover to the FMI, as such, it will be mountable on any standard balloon flight without having to introduce major design changes. If a battery as a power unit where introduced, hence less bags could be carried (around five bags) in this potential future setup, see Figure \ref{battery_distribution}.
}
...
\DIFdelbegin \DIFdel{As it is mentioned }\DIFdelend \DIFaddbegin \DIFadd{The main mechanical characteristics of the experiment are summarized }\DIFaddend in Table \ref{table:experiment-summary}, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{where the values are based on the reference axis shown in Figure \ref{COG}. The }\DIFaddend Center Of Gravity for the whole experiment \DIFdelbegin \DIFdel{appears }\DIFdelend \DIFaddbegin \DIFadd{is determined }\DIFaddend to be located just on base of the third level of The Brain which coincides with the location of the electronics PCB. This outcome is quite advantageous in terms of stability for one of the most sensitive subsystems of the experiment in terms of shakes and loads. \DIFdelbegin \DIFdel{Notice that the values for the center of gravity are based on the reference axis shown in Figure \ref{COG}. Also, the table }\DIFdelend \DIFaddbegin \DIFadd{It should also be noted that the }\DIFaddend weights of the \DIFaddbegin \DIFadd{table for the }\DIFaddend boxes and, therefore, the whole experiment, are increased by a \DIFaddbegin \DIFadd{safety margin of }\DIFaddend $10\%$\DIFdelbegin \DIFdel{of safety margin }\DIFdelend .
...
\DIFdelbegin \DIFdel{The frame of the structure, shown in Figure \ref{structure}, is built by the aforesaid aluminum strut profiles which are connected to each other with cubic connectors 20/3 that provide better strength and stability for the whole structure. These two components are connected by M6x16 normalized }\DIFdelend \DIFaddbegin \DIFadd{Moreover, other considerations such as electrical and, especially, thermal conductivity are also be a concern since the experiment will fly up to \mbox{%DIFAUXCMD
$25\ km$
}%DIFAUXCMD
...
\DIFadd{For this purpose, two boxes built with straight frames have been chosen as the best option as shown in Figure \ref{fig:structure}. The frame of these boxes will be strut profiles made of aluminum, with a characteristic cross-section of \mbox{%DIFAUXCMD
$20\times20\ mm$
}%DIFAUXCMD
...
\DIFdelbegin \DIFdel{The two-box design will allow ease of access and manipulation of both the CAC and AAC subsystems, see Figure \ref{fig:3D_tubular_render}. In addition, the AAC sampling system is designed to be re-usable for future handover to FMI, as such, it will be mountable on any standard balloon flight without having to introduce major design changes. The latter would mean to introducing a battery as a power unit, hence less bags could be carried (around five bags) in this potential future setup, see Figure \ref{battery_distribution}}\DIFdelend \DIFaddbegin \smallskip

\subsubsection{\DIFadd{Walls and Protections}}
\label{sec:4.4.2}

\DIFadd{Since the experiment will be placed close to the outside of the gondola, it is very exposed to both external elements impacts and also possible broken parts from other experiments in the gondola due to unexpected rapid movements, and a probable hard landing impact. Therefore, the experiment box will be shielded with removable aluminum walls along with a thick layer of Styrofoam attached to each wall. This thickness varies from two to three centimeters in the AAC box, and five centimiters to protect the AirCore. Besides protection, the thickness of the styrofoam is also motivated by thermal control issues.
%DIF >  which is explained more in detail in Section \ref{sec:4.6.6}.
}

\DIFadd{To mount the experiment a combination of three different elements will be used, as shown in Figure \ref{fig:wall_attach}. The walls will be screwed to the Variofix blocks by means of \mbox{%DIFAUXCMD
$M4\times8$
}%DIFAUXCMD
bolts. In between the aluminum walls and the bolts, a \mbox{%DIFAUXCMD
$M4$
}%DIFAUXCMD
retainer ring will be placed to improve the fixation of each spot. Four fixation points for each wall have been considered sufficient to keep the experiment safe from any impact. However, double the amount of fixation points, eight, will be used in the more exposed walls that are facing outside the gondola}\DIFaddend .
...
%DIF <     \label{strucutre}
%DIF < \end{figure}
\DIFaddbegin \DIFadd{The styrofoam sheets will be attached to the aluminum walls with polyamide washers screwed along both layers.
}\DIFaddend 
...
\DIFdelend \DIFaddbegin \DIFadd{Tables \ref{table:wall_aluminum} and \ref{table:wall_styrofoam} show the main properties of the materials used to build the walls of the boxes.
}\DIFaddend 
...
\subsubsection{\DIFadd{CAC Box}}

\DIFaddend The CAC subsystem is designed to fit a \DIFdelbegin \DIFdel{300 }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
$300$
}%DIFAUXCMD
}\DIFaddend m stainless steel coiled tube, a solenoid valve \DIFdelbegin \DIFdel{governing }\DIFdelend \DIFaddbegin \DIFadd{controlling }\DIFaddend it, interfaces, an air filter and three temperature sensors. A schematic of this subsystem can be seen in Figure \ref{fig:CAC-schematic}. The CAC consists \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend a combination of a \DIFdelbegin \DIFdel{200 }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
$200$
}%DIFAUXCMD
}\DIFaddend m coiled tube of \DIFdelbegin \DIFdel{1/8 in. and a 100 }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
$1/8$
}%DIFAUXCMD
inches diameter and a \mbox{%DIFAUXCMD
$100$
}%DIFAUXCMD
}\DIFaddend m coiled tube of \DIFdelbegin \DIFdel{1/4 in}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
$1/4$
}%DIFAUXCMD
inches diameter}\DIFaddend . The outlet of the CAC is sealed with a quick connector provided by FMI. The inlet will be sealed \DIFdelbegin \DIFdel{in }\DIFdelend the same way but it will be \DIFdelbegin \DIFdel{open by means of another interfaced plugged to }\DIFdelend \DIFaddbegin \DIFadd{opened by another interface plugged into }\DIFaddend the quick connector. A filter is placed between this orifice and the solenoid valve. The filter will be custom made by FMI. The set up is a tube containing magnesium perchlorate powder with stone wool at both ends of the tube. It will ensure that no moisture will enter the coil during any testing or sampling. Another tube is attached to the solenoid valve that goes outside the box, thus having a direct outside outlet and inlet for the whole CAC system, as seen in Figure \ref{fig:CAC-cad-model}.
...
%DIF <  mechanical issues and gondola constraints 
}\DIFdelend \DIFaddbegin \DIFadd{cable will link the respective D-sub connector on each box.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{The AAC Subsystem consists of six 3 L sampling bags. }\DIFdelend \DIFaddbegin \DIFadd{The AAC box has been designed to be as compact as possible. An analysis regarding the variation of the bags dimensions to different sampled volume, has been made and summarized in Appendix \ref{dimensions-bags}. From these results it was shown that the AAC subsystem is able to fit six \mbox{%DIFAUXCMD
$3\ L$
}%DIFAUXCMD
sampling bags together The Brain that includes the pneumatic system and the electronic box. }\DIFaddend Each bag will have a dedicated valve in the Valve Center (VC) to allow emptying and filling processes as well as to close the bag when needed. The bags will be \DIFdelbegin \DIFdel{placed vertically and will have }\DIFdelend \DIFaddbegin \DIFadd{hanging from a bar that will be attached to the structure frame by }\DIFaddend two anchor points \DIFdelbegin \DIFdel{: }\DIFdelend on the top\DIFdelbegin \DIFdel{through a  multiple anchor interface (see Figure \ref{anchor_bags}) and on the bottom by means of the tubes connecting them to the valves. }%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend Level 1 \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{of The Brain is lying on the base wall styrofoam. It contains the beginning of the pneumatic sampling system. The inlet tube passes through the wall and interfaces with }\DIFaddend the \DIFdelbegin \DIFdel{base of }\DIFdelend \DIFaddbegin \DIFadd{filter. From here the system continues through the pump, airflow sensor and to Level 2. The reason for having the pump in Level 1 is to have the minimum vibration transmitted to the other components. The pump will have two heaters close by that will be used to regulate its temperature. This can be seen as two black rectangular sheets underneath the pump in Figure \ref{level_1}.
}

\DIFadd{The second level of The Brain is responsible for the distribution of the air to the selected sampling bag. The manifold with 8 solenoid valves is the main component. From here, the tubes connect with the bags. A T-Union connection is used just before the bag valve. This interface allows }\DIFaddend the \DIFdelbegin \DIFdel{AAC box and together with Level 2 contain the main pneumatic system. This is commanded by the electronics in the top level . This distribution allows easy access to the PCB from the top and provides the physical desired separation between electronics and pneumatic circuit. }\DIFdelend \DIFaddbegin \DIFadd{pre-flight flushing of the tubes connecting with the valves as explained previously. 
}\DIFaddend 

\DIFaddbegin \smallskip
\DIFadd{The flushing valve is the responsible to ensure a proper flushing of the system before each sampling period. From the flushing valve, a outlet tube (in red) reaches the outside environment. This can be seen in Figure \ref{level_2}.
}

\DIFaddend The \DIFdelbegin \DIFdel{fact of having different levels implies the need of having dedicated holes to bring }\DIFdelend \DIFaddbegin \DIFadd{OBC and its external elements will be allocated in the third level of The Brain. The PCB will be fixed to the aluminum plate by means of 5 standoffs. As shown in Figure \ref{level_3}, it has a hole, as well as the level plate, to collect }\DIFaddend all the wires \DIFdelbegin \DIFdel{to the third level and the tube to the second level. 
This is shown in the dedicated section }\DIFdelend \DIFaddbegin \DIFadd{connecting with levels 1 and 2. This level has its own outside wall which contains the electrical interfaces. The latter allows to open the wall without having to remove all the sockets attached with screws and a female in the inside of the wall. The styrofoam shielding of The Brain has a hole at this height to allow the temperature sensors wires to reach the inside of the AAC Box. 
}

\DIFadd{A more detailed content of the components }\DIFaddend for each level \DIFdelbegin \DIFdel{which can be seen in Figure \ref{brain_lateral}.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{is summarized in Appendix \ref{list-of-components-brain}.
}\DIFaddend \begin{figure}[H]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{4-experiment-design/img/Mechanical/Brain_Lateral.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[b]{0.3\textwidth}
    ...
    \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Lateral }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Isometric }\DIFaddendFL View of \DIFdelbeginFL \DIFdelFL{The Brain.}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Level 1.}\DIFaddendFL }
    \DIFdelbeginFL %DIFDELCMD < \label{brain_lateral}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{level_1}
    \end{subfigure}
    \DIFaddFL{~
...
\DIFaddbegin \DIFadd{This distribution allows easy access to the PCB from the bottom and provides the physical desired separation between electronics and pneumatic circuit.
...
\DIFaddend \smallskip
The bulk dimensions of \DIFdelbegin \DIFdel{the brain }\DIFdelend \DIFaddbegin \DIFadd{The Brain }\DIFaddend are 260 mm long, 150mm wide and 290 mm high. If the shielding styrofoam walls are taken into account, the dimensions are 290 mm long, 180 mm wide and 300 mm high.
...
\smallskip
In order to allocate the electrical interfaces required (E-Link, Power Supply and D-Sub Connectors) as well as to allow the tubes of the sampling system to reach the outside environment, the outside facing wall is divided in three pieces. Two small pieces \DIFaddbegin \DIFadd{of aluminium sheet }\DIFaddend are fixed to \DIFdelbegin \DIFdel{the Brain structure. The latter provides easiness of manipulation }\DIFdelend \DIFaddbegin \DIFadd{The Brain structure, as seen in Figure \ref{brain_isometric}. This will make it easy to manipulate }\DIFaddend when having to open the box wall since the little pieces containing the interfaces and the tubes holes, will remain attached. The bottom piece covers Level 1 and 2 while the other, which contains the electrical connections, protects Level 3. These pieces have the same layout as the main wall. 
...
In order to be able to collect separated samples of air, a pneumatic \DIFdelbegin \DIFdel{system }\DIFdelend \DIFaddbegin \DIFadd{subsystem }\DIFaddend has been developed. The schematics and components of this can be seen in Figure \ref{pneumatic_system}. The \DIFdelbegin \DIFdel{circuit }\DIFdelend \DIFaddbegin \DIFadd{system }\DIFaddend is formed by \DIFdelbegin \DIFdel{a total of 81 }\DIFdelend \DIFaddbegin \DIFadd{almost \mbox{%DIFAUXCMD
$100$
}%DIFAUXCMD
}\DIFaddend components located inside \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend Brain and the AAC Box. 

\DIFaddbegin \DIFadd{In between these components, the same Sulfinert-treated stainless steel tubing as the ones used for the Inlet/Outlet pipes explained in Section \ref{subsec:pipes} have been chosen. According to the datasheet, the minimum suggested bend radius for the chosen diameter tube is \mbox{%DIFAUXCMD
$10.2\ cm$
}%DIFAUXCMD
...
\DIFaddend The schematic for the pneumatic system can be seen in Figure \ref{pneumatic_system}. The air is sucked from the outside through the inlet tube (No.1), turquoise in Figure \DIFdelbegin \DIFdel{\ref{level_1_pneumatic_system_top_view}}\DIFdelend \DIFaddbegin \DIFadd{\ref{pneumatic_system_cad}}\DIFaddend , and it goes through the filter (No.3) inside the pump (No.7). From here, it passes through the airflow sensor (No.11), which allows to monitor the flow rate, before changing to Level \DIFdelbegin \DIFdel{2 (Figure \ref{level_2_pneumatic_system_top_view}). There, the }\DIFdelend \DIFaddbegin \DIFadd{2. Thereafter the air passes through the }\DIFaddend sensor box (No.15) \DIFdelbegin \DIFdel{containing }\DIFdelend \DIFaddbegin \DIFadd{which contains }\DIFaddend three pressure sensors \DIFdelbegin \DIFdel{and one humidity sensor, is the first component the air passes through }\DIFdelend before getting to the \DIFdelbegin \DIFdel{8 }\DIFdelend \DIFaddbegin \DIFadd{six }\DIFaddend stations manifold (No.19). It is in here where the air is directed to the desired bag (No.31) thanks to its dedicated solenoid valve (No.26).
\DIFaddbegin 

\DIFaddend When flushing the \DIFdelbegin \DIFdel{circuitry }\DIFdelend \DIFaddbegin \DIFadd{pneumatic system }\DIFaddend before each sampling period, the flushing valve (No.23) is opened so \DIFaddbegin \DIFadd{that }\DIFaddend the outlet of the system is \DIFdelbegin \DIFdel{opened so }\DIFdelend \DIFaddbegin \DIFadd{open and }\DIFaddend new air runs through the main \DIFdelbegin \DIFdel{circuit}\DIFdelend \DIFaddbegin \DIFadd{part of the pneumatic system}\DIFaddend . 
...
\begin{enumerate}
    \item The Brain level 3, where the PCB is located with the Arudino and shield, two 24 V DC-DC, \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{two }\DIFaddend 12 V \DIFdelbegin \DIFdel{voltage regulator}\DIFdelend \DIFaddbegin \DIFadd{DC-DC}\DIFaddend , one temperature sensor, 11 MOSFETs and \DIFdelbegin \DIFdel{14 }\DIFdelend \DIFaddbegin \DIFadd{16 }\DIFaddend LEDs.
    \item The Brain level 2, where the valve manifolds with six sampling valves\DIFdelbegin \footnote{\DIFdel{There will be eight valves connected mechanically in the manifold to ensure it is sealed but only six will be connected to the bags and electronics.}%DIFDELCMD < \label{fn:extravalve}%%%
}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend , the flushing valve, \DIFdelbegin \DIFdel{one heater }\DIFdelend \DIFaddbegin \DIFadd{two heaters }\DIFaddend and one temperature sensor are located.
    \item The Brain level 1, where the pump, \DIFdelbegin \DIFdel{airflow sensor and sensor box containing 3 pressure sensors, 1 }\DIFdelend \DIFaddbegin \DIFadd{two heaters, airflow sensor, one }\DIFaddend temperature sensor and \DIFdelbegin \DIFdel{1 humidity }\DIFdelend \DIFaddbegin \DIFadd{an inline pressure }\DIFaddend sensor are located.
...
All of the power distribution will take place on the PCB using two 24 V DC-DC \DIFaddbegin \DIFadd{and two 12 V DC-DC }\DIFaddend converters in parallel with a forwarding diode\DIFdelbegin \DIFdel{which feed a 12 V voltage regulator}\DIFdelend .  
\begin{itemize}
...
}\DIFaddend for the airflow sensor and Arduino due.

    \item $3.3 \, V$ for the temperature \DIFdelbegin \DIFdel{, pressure and humidity }\DIFdelend \DIFaddbegin \DIFadd{and pressure }\DIFaddend sensors. 

\end{itemize}

The voltage available from gondola power is 28.8 V, therefore the heaters have been connected directly to the main power supply. For the rest of the components, two 24 V \DIFaddbegin \DIFadd{and two 12 V }\DIFaddend DC-DCs in parallel has been used to make sure if one of them fails then the other can take over. The circuitry can be seen in Figure \ref{fig:dc-dc-redun}. All the valves and the pump are then powered through the 24 V DC-DCs. To step down the voltage from \DIFdelbegin \DIFdel{24 }\DIFdelend \DIFaddbegin \DIFadd{28.8 }\DIFaddend V to 12 V to power the airflow sensor and the Arduino, \DIFdelbegin \DIFdel{a voltage regulator }\DIFdelend \DIFaddbegin \DIFadd{two 12 V DC-DCs in parallel }\DIFaddend has been used \DIFdelbegin \DIFdel{at the output of the DC-DCs}\DIFdelend \DIFaddbegin \DIFadd{for the redundancy purposes}\DIFaddend . Finally, to power the temperature \DIFdelbegin \DIFdel{, humidity }\DIFdelend and pressure sensors, 3.3 V is required which is supplied by the Arduino board. 
...
\begin{figure}[H]
    \begin{align*}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=16cm]{4-experiment-design/img/DCDC-converter-redundancy.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=13cm]{4-experiment-design/img/DCDC-converter-redundancy.png}
    \DIFaddendFL \end{align*}
    \caption{Schematic Showing the DC-DC Redundancy of Both 24 V  \DIFdelbeginFL \DIFdelFL{DC-DC Converters }\DIFdelendFL and the 12 V \DIFdelbeginFL \DIFdelFL{Voltage Regulator}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{DC-DC Converters.}\DIFaddendFL }\label{fig:dc-dc-redun}
\end{figure}
...
\DIFaddbegin \DIFadd{The PCB was made using Eagle software. The traces have a width designed to fit the IPC-2221 standards\mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{E11 }%DIFDELCMD < & %%%
\DIFdel{Humidity Sensor }%DIFDELCMD < & %%%
\DIFdel{-40 }%DIFDELCMD < & %%%
\DIFdel{85 }%DIFDELCMD < & %%%
\DIFdel{-40 }%DIFDELCMD < & %%%
\DIFdel{125 }%DIFDELCMD < & %%%
\DIFdel{-10 }%DIFDELCMD < & %%%
\DIFdel{20 }%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\DIFdel{E11 }\DIFdelend \DIFaddbegin \DIFadd{E9  }\DIFaddend & \DIFdelbegin \DIFdel{Humidity sensors }\DIFdelend \DIFaddbegin \DIFadd{Temperature sensor }\DIFaddend & 3.3 & \DIFdelbegin \DIFdel{0.0005   }\DIFdelend \DIFaddbegin \DIFadd{0.28 }\DIFaddend & \DIFdelbegin \DIFdel{1.65\mbox{%DIFAUXCMD
$\times10^{-3}$
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{0.011  }\DIFaddend & \DIFdelbegin \DIFdel{0.01 }\DIFdelend \DIFaddbegin \DIFadd{0.11   }\DIFaddend \\ \hline
\DIFdelbegin \DIFdel{E8  }%DIFDELCMD < & %%%
\DIFdel{Voltage Regulator   }%DIFDELCMD < & %%%
\DIFdel{24   }%DIFDELCMD < & %%%
\DIFdel{8 }%DIFDELCMD < & %%%
\DIFdel{0.028 }%DIFDELCMD < & %%%
\DIFdel{0.28 }%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
\DIFdelend 
...
\DIFdelbeginFL \DIFdelFL{Humidity sensor-OBC            }%DIFDELCMD < & %%%
\DIFdelFL{I2C                }%DIFDELCMD < & %%%
\DIFdelFL{Arduino I2C }%DIFDELCMD < \\
%DIFDELCMD < %%%
...
\\
\DIFadd{The telecommand contains the following services:
}\begin{itemize}
    \item \DIFadd{Changing instrument modes
    }\item \DIFadd{Manually control valves, pump, and heaters
}\end{itemize}

\DIFadd{Furthermore, telemetry contains the services below:
}\begin{itemize}
    \item \DIFadd{Data from temperature, pressure and airflow sensor
...
\DIFadd{P}\DIFaddend .12 & \DIFdelbegin \DIFdel{The experiment }\textit{\DIFdel{shall}} %DIFAUXCMD
\DIFdel{collect data on the humidity. }%DIFDELCMD < & %%%
\DIFdel{A, T }%DIFDELCMD < & %%%
\DIFdel{24, 31  }%DIFDELCMD < & %%%
\DIFdel{Pass by similarity\textsuperscript{\ref{fn:sensor-libraries}}}%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
\DIFdel{F.13 }%DIFDELCMD < & \st{The experiment \textit{shall} measure the temperature inside the AAC Valve Box.} %%%
\DIFdel{\textsuperscript{\ref{fn:unnecessary-requirement}}}%DIFDELCMD < &%%%
\DIFdel{- }%DIFDELCMD < & %%%
\DIFdel{-}%DIFDELCMD < & \\ \hline
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{P.14 }%DIFDELCMD < & %%%
\DIFdel{The accuracy of the ambient humidity measurements }\textit{\DIFdel{shall}} %DIFAUXCMD
\DIFdel{be +-3\%.                                                                                                         }%DIFDELCMD < &       %%%
\DIFdel{I         }%DIFDELCMD < &  %%%
\DIFdel{-           }%DIFDELCMD < & %%%
\DIFdel{Pass        }%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
\DIFdel{P.15 }%DIFDELCMD < & \st{The accuracy of the AAC Valve Box temperature measurements \textit{shall} be +3.5/-2$\degree$C(max).}%%%
...
\DIFadd{P.28 }& \DIFadd{The sampling rate of the pressure sensor }\textit{\DIFadd{shall}} \DIFadd{be 1 Hz.                                                                                    }&         \DIFadd{A,T     }& \DIFadd{10            }&  \DIFadd{Analysis passed, see Section \ref{sec:4.8.2}      }\\ \hline
\DIFadd{P.29 }& \DIFadd{The sampling rate of the airflow sensor }\textit{\DIFadd{shall}} \DIFadd{be 1 Hz.                                                                                    }&         \DIFadd{A,T     }& \DIFadd{10            }&  \DIFadd{Analysis passed, see Section \ref{sec:4.8.2}      }\\ \hline
\DIFadd{P.30 }& \DIFadd{The accuracy of the pressure measurements inside the tubing and sampling bags }\textit{\DIFadd{shall}} \DIFadd{be -0.005/+0.005 bar for 25\mbox{%DIFAUXCMD
$\degree$
}%DIFAUXCMD
C.                                                                              }&        \DIFadd{R      }&  \DIFadd{-          }& \DIFadd{Pass from data sheet       }\\ \hline

\DIFaddend D.1  & The experiment \textit{shall} operate in the temperature profile of the BEXUS vehicle flight and launch.\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{BexusManual}                                                                         }\hspace{0pt}%DIFAUXCMD
}\DIFaddend &       A, T       & 5            & Verification is ongoing.     \\ \hline
D.2  & The experiment \textit{shall} operate in the vibration profile of the BEXUS vehicle flight and launch.\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{BexusManual}                                                                          }\hspace{0pt}%DIFAUXCMD
...
\\ \hline
D.18 & The  experiment \textit{shall} operate  in  the  pressure  profile  of  the BEXUS flight.\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{BexusManual}                                                                              }\hspace{0pt}%DIFAUXCMD
}\DIFaddend &    A, T         & 4, 18, 30 &  Pump Passed Test 18     
\\ \hline
D.19 & The  experiment \textit{shall} operate  in  the  vertical  and  horizontal  acceleration  profile  of  the BEXUS flight.\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{BexusManual}                                                                              }\hspace{0pt}%DIFAUXCMD
...
\textbf{Test Facility} & \DIFdelbeginFL \DIFdelFL{Esrange Space Centre }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{IRF, Kiruna }\DIFaddendFL TBC \\ \hline
\textbf{Tested Item} & The entire experiment \\ \hline
...
\multirow{2}{*}{\textbf{\begin{tabular}[c]{@{}l@{}}Test Level/ Procedure \\ and Duration\end{tabular}}} & Test procedure: \DIFdelbeginFL \DIFdelFL{Use a shake table in the university facilities to test both random and sinusoidal vibrations. The boxes will be tested individually and attached together. In order to inspect the response of the inside elements, the test will also be done without the walls. Backup plan in case the shake table is not available: mount the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Mount the }\DIFaddendFL experiment on the back of a car/trailer \DIFaddbeginFL \DIFaddFL{in the same way it will be mounted on the gondola }\DIFaddendFL and drive over bumpy or rough terrain. Afterwards, check the experiment for functionality and structural integrity.\\ & Test duration: 2 hours \\ \hline
\textbf{Test Campaign Duration} & 1 week \\ \hline
\textbf{Test Campaign Date} & September \\ \hline
\textbf{Test Completed} & NO \\ \hline
\end{tabular}
...
%DIF > Use a shake table in the university facilities to test both random and sinusoidal vibrations. The boxes will be tested individually and attached together. In order to inspect the response of the inside elements, the test will also be done without the walls.
...
\DIFdelbeginFL \DIFdelFL{T+45 min                         }%DIFDELCMD < & %%%
\DIFdelFL{15 km                              }%DIFDELCMD < & %%%
\DIFdelFL{Experiment stops emptying the tubes              }%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
\DIFdelendFL %DIF > T+45 min                         & 15 km                              & Experiment stops emptying the tubes              \\ \hline
