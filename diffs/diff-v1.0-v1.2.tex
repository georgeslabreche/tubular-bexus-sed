...
\begin{longtable}{|p{0.1\textwidth}| p{0.22\textwidth} |p{0.28\textwidth} |p{0.35\textwidth}|}\hline
    ...
    \DIFaddbegin \DIFadd{1-1     }&   \DIFadd{2018-01-25   }& \DIFadd{1.1, 2.2, 2.3, 3.3.3, 3.5, 4.1, 4.4.2, 4.5, 4.6, 4.7, 6.1.5, 6.1.6, 6.2, 6.4, 7.3.1. }& \DIFadd{Incorporated feedback from supervising professor.}\\ 
    \DIFadd{1-2     }&   \DIFadd{2018-03-07   }& \DIFadd{2.1, 2.3, 2.4, 2.5, 3.5, 4.1, 4.4, 4.6, 4.8, 5.2, 6.1.4, 6.2, 6.3, 6.4, appendix: B, C.     }&    \\\DIFaddend \hline 
...
\DIFdel{not be greater than 7 Hz.
    ...
    \item The accuracy of the ambient humidity measurements \textit{shall} be ±\DIFdelbegin \DIFdel{2 percent. }\DIFdelend \DIFaddbegin \DIFadd{3\%. \mbox{%DIFAUXCMD
\cite{Humiditysensor}
    }\hspace{0pt}%DIFAUXCMD
}\DIFaddend \item \DIFdelbegin \DIFdel{The accuracy of the AAC Valve Box temperature measurements }\textit{\DIFdel{shall}} %DIFAUXCMD
\DIFdel{be +3.5/-2°C(max).
    ...
    \DIFaddend \item The air intake rate of the air pump \textit{shall} be \DIFaddbegin \DIFadd{minimum }\DIFaddend 3L/min.
...
FMI is a key partner in the TUBULAR project, its scientific experts will advise and support the TUBULAR project by sharing knowledge, experience, and granting accessibility of equipment. \DIFaddbegin \DIFadd{As per the agreement shown in Appendix \ref{sec:appG}, }\DIFaddend FMI will provide the TUBULAR team with the AirCore stainless tube component of CAC subsystem as well as the post-flight gas analyzer. \DIFaddbegin \DIFadd{This arrangement requires careful considerations on the placement of the experiment in order to minimize hardware damage risks. }\DIFaddend These contributions will result in significant cost savings regarding equipment and component procurement.
...
\item%DIFAUXCMD
\DIFdel{AAC sampling bags shall be flushed.
    }\DIFdelend \DIFaddbegin \DIFadd{AAC' control valve shall be closed.
    }\DIFaddend \end{itemize}
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
$t_1$
}%DIFAUXCMD
...
$p_2$
    ...
    \DIFaddbegin \item \DIFadd{CAC valve shall be opened.
    }\DIFaddend \item AAC valves shall be \DIFdelbegin \DIFdel{closed}\DIFdelend \DIFaddbegin \DIFadd{closed.
    }\item \DIFadd{CAC tube shall be flushed.
    }\item \DIFadd{AAC' control valve shall be open}\DIFaddend .
    \end{itemize}
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
$t_2$
}%DIFAUXCMD
...
The CAC \DIFdelbegin \DIFdel{Subsystem is designed around a the }\DIFdelend \DIFaddbegin \DIFadd{subsystem is designed to fit a }\DIFaddend 300-meter coiled tube\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the valve governing it \DIFdelbegin \DIFdel{. It will be placed at the bottom of }\DIFdelend \DIFaddbegin \DIFadd{and a temperature sensor. To determine its positioning inside the gondola and the experiment box, some mechanical issues and gondola constraints must be considered.
}

\smallskip
\DIFadd{Firstly, it is possible to identify the interface to attach }\DIFaddend the experiment box \DIFdelbegin \DIFdel{providing a stable location of the CoG. The }\DIFdelend \DIFaddbegin \DIFadd{to the gondola as one of the most critical points in terms of mechanics performance. In the worst case scenario, with a heavy experiment and without a proper study of the aforesaid interface, shear in the screws could be produced after a violent landing stress. Since the CAC will be the heaviest component in the whole experiment, its location and orientation will affect directly the stress analysis of the structure. The larger the distance to the fixed points, the bigger the momentum produced by the component. Nevertheless, due to fast recovery implementation, the CAC tube will be placed vertically. Therefore, its dedicated box will be properly attached to the AAC box by means of 4 anchor points, in black in Figure \ref{overview}. The fast recovery then will only imply unscrewing 4 screws and disconnecting a wire. 
}

\smallskip
\DIFadd{In order to command the valve, a wire will go out from the box and connected to the electronics interface panel located on an AAC box wall.
}

\smallskip
\DIFadd{In addition, to avoid sample contamination with standstill air inside the gondola, the }\DIFaddend coil will have a direct outside inlet and outlet by means of an extension tube reaching further from the gondola’s limits\DIFdelbegin \DIFdel{.
...
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.7\textwidth]{4-experiment-design/img/valve_collector.jpg}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{7                            }\DIFdelend \DIFaddbegin \DIFadd{10                            }\DIFaddend & \DIFaddbegin \DIFadd{DC-DC step converter                                                 }& \DIFadd{-40                       }& \DIFadd{85                       }& \DIFadd{-50                      }& \DIFadd{125                      }\\ \hline
\DIFadd{11                           }& \DIFaddend HDC2010 Low Power Humidity Digital Sensors                           & -40                    & 85                    & -40                   & 125                   \\ \hline
\DIFdelbegin \DIFdel{8                            }\DIFdelend \DIFaddbegin \DIFadd{12                           }\DIFaddend & Industrial temperature microSD XCUHS-I 8GB                           & -40                    & 85                    & -40                   & 85                    \\ \hline
...
%DIFDELCMD < %%%
\DIFdel{10                           }\DIFdelend \DIFaddbegin \DIFadd{= Thermal conductivity of the pump's inlet valve(s) made of PPS
    }\item \DIFadd{\mbox{%DIFAUXCMD
$k_{EPDM}$
}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{11                           }\DIFdelend \DIFaddbegin \DIFadd{= Temperature at the manifold's outlet(s)
    }\item \DIFadd{\mbox{%DIFAUXCMD
$T_{heated}$
}%DIFAUXCMD
}\DIFaddend &\DIFdelbegin \DIFdel{Micro Air Pump CMP30-3PW                                                             }\DIFdelend \DIFaddbegin \DIFadd{= Temperature of the pump at its diaphragm face
    }\item \DIFadd{\mbox{%DIFAUXCMD
$L_{outhalf}$
}%DIFAUXCMD
}\DIFaddend &\DIFdelbegin \DIFdel{0                       }\DIFdelend \DIFaddbegin \DIFadd{= Length of the outer half of the manifold
    }\item \DIFadd{\mbox{%DIFAUXCMD
$L_{inhalf}$
}%DIFAUXCMD
...
}\DIFaddend &\DIFdelbegin \DIFdel{0                      }\DIFdelend \DIFaddbegin \DIFadd{= Thermal conductivity of Nylon-6
    }\item \DIFadd{\mbox{%DIFAUXCMD
$k_{Kynar}$
}%DIFAUXCMD
}\DIFaddend &\DIFdelbegin \DIFdel{50                      }%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
...
\DIFdelend \DIFaddbegin \multicolumn{1}{c|}{1811}                    \DIFaddend & \DIFdelbegin %DIFDELCMD < \multicolumn{1}{c|}{44.3}                 %%%
...
Before the launch, both CAC and AAC will be filled with  an inert gas\DIFdelbegin \DIFdel{, making sure that during ascent phase both CAC and AAC will be sufficiently emptied before sampling. For the experiment will be used nitrogen, which is referred to as and used as an }\DIFdelend \DIFaddbegin \DIFadd{. All the bags will be manually emptied before flight. Flushing of the CAC tube will be done passively through the progressive decrease in air pressure during the balloon's ascent phase. Nitrogen will be used as the }\DIFaddend inert gas. Nitrogen's triple bond is very strong and requires a lot of energy to break those bonds and participate in a reaction. That is why nitrogen is used, despite the fact that is not truly inert like most noble gases.
...
\DIFaddbegin \DIFadd{But considering the fact that the pump will not have the same efficiency at higher altitudes, the sampling time may be longer and the airflow intake rate may be higher. The exact numbers will be included in the upcoming version of the SED.  
}\DIFaddend 
